...
public class ScanFileServiceImpl implements IScanFileService {
    ...
    private boolean flagValidacionVacio = true;
    ...
    public UploadExcelFileResponse saveData() {
        ...
        if (flagValidacionEstructura) {
            ...
            if (vendorIdentification == null || vendorIdentification.equals("0")) {
                ...
                responseMessages.add(
                        "El número de identificación del proveedor no puede contener letras o caracteres especiales");
            }
        ...
        }else if(flagValidacionVacio == false) {
            flag = false;
            responseMessages.add("Archivo vacio");
            flagValidacionVacio = true;
            }else{
                flag = false;
                responseMessages.add("El formato del archivo no es correcto, verifique la estructura del acrhivo");
                flagValidacionEstructura = true;
            }
        //
        if (!flag) {
            ...
            if (contract.isEmpty()) {
                ...
                if (contract.get().getInitialDate() == null) {
                    messageType = MessageType.VALIDATION;
                    responseMessages.add("La fecha de inicio está vacía en la base de datos");
                } else if (contract.get().getFinalDate() == null) {
                    messageType = MessageType.VALIDATION;
                    responseMessages.add("La fecha de terminación está vacía en la base de datos");
                } else {
                    if (!initialDate.toString().equals(contract.get().getInitialDate().toString())) {
                        messageType = MessageType.VALIDATION;
                        responseMessages.add("La fecha de inicio no coincide con la registrada en la base de datos");
                    } else if (!finalDate.toString().equals(contract.get().getFinalDate().toString())) {
                        messageType = MessageType.VALIDATION;
                        responseMessages
                                .add("La fecha de terminación no coincide con la registrada en la base de datos");
                    } else {

                        Score objScore = scoreRepository.findById(contract.get().getId()).get();
                        boolean isEvaluationRegister = objScore.getCreateTime().equals(objScore.getUpdateTime()) ? false
                                : true;
                        if (isEvaluationRegister) {
                            ...
                            var qualityCriteria = this.criteriaRepository
                                    .findByNameAndCriteriaType("Calidad", criteriaType)
                                    .orElseThrow();
                            var executionCriteria = this.criteriaRepository
                                    .findByNameAndCriteriaType("Ejecucion", criteriaType)
                                    .orElseThrow();
                            var complianceCriteria = this.criteriaRepository
                                    .findByNameAndCriteriaType("Cumplimiento", criteriaType)
                                    .orElseThrow();
    ...
    public List<UploadExcelFileResponse> processMassiveFile(MultipartFile file) throws IOException {
        ...
        List<String> encabezadosArchivo = Arrays.asList("NÚMERO DE CONTRATO",
                "FECHA SUSCRIPCIÓN CONTRATO (AAAA/MM/DD)", "CLASE DE CONTRATO", "OBJETO DEL CONTRATO",
                "CONTRATISTA : TIPO IDENTIFICACIÓN", "CONTRATISTA : NÚMERO DE CÉDULA o RUT",
                "CONTRATISTA : NÚMERO DEL NIT",
                "CONTRATISTA : NOMBRE COMPLETO", "SUPERVISOR : NOMBRE COMPLETO", "FECHA INICIO CONTRATO (AAAA/MM/DD)",
                "FECHA TERMINACIÓN CONTRATO (AAAA/MM/DD)", "EVALUACIÓN");
        ...
        Row firstRowValidation = sheet.getRow(0);
        // Lecutra Encabezados
        final Cell cell1A,cell1B,cell1C,cell1D,cell1E,cell1F,cell1G,cell1H,
            cell1I,cell1J,cell1K,cell1L;
        if(firstRowValidation != null){
            cell1A = sheet.getRow(0).getCell(0);
            cell1B = sheet.getRow(0).getCell(1);
            cell1C = sheet.getRow(0).getCell(2);
            cell1D = sheet.getRow(0).getCell(3);
            cell1E = sheet.getRow(0).getCell(4);
            cell1F = sheet.getRow(0).getCell(5);
            cell1G = sheet.getRow(0).getCell(6);
            cell1H = sheet.getRow(0).getCell(7);
            cell1I = sheet.getRow(0).getCell(8);
            cell1J = sheet.getRow(0).getCell(9);
            cell1K = sheet.getRow(0).getCell(10);
            cell1L = sheet.getRow(0).getCell(11);
        }else{
            cell1A=cell1B=cell1C=cell1D=cell1E=cell1F=cell1G=cell1H=
            cell1I=cell1J=cell1K=cell1L = null;
        }
